...
This produces `aevum.img` — a bootable floppy image with a 512-byte bootloader and an 8 KB kernel. The kernel boots into protected mode, initializes VGA text mode (80×25), a polling keyboard driver, and a shell with 11 commands.
...
| `kernel.asm` | Kernel (PMode, VGA, keyboard, shell, 11 commands) |
...
Результат — `aevum.img`: загрузочный образ дискеты с загрузчиком (512 байт) и ядром (8 КБ). Ядро загружается в защищённый режим, инициализирует VGA text mode (80×25), клавиатуру (опрос) и оболочку с 11 командами.
...
| `kernel.asm` | Ядро (PMode, VGA, клавиатура, shell, 11 команд) |
